...
@Component
public class AreaFilter implements ActivityFilter{

	@Override
	public boolean apply(ActivitySearchRequest activitySearchRequest) {
		return activitySearchRequest.getAreaIds() != null && !activitySearchRequest.getAreaIds().isEmpty();
	}

	@Override
	public BooleanExpression filter(ActivitySearchRequest activitySearchRequest, BooleanExpression predicate) {
		BooleanExpression finalPredicate = QActivity.activity.area.id.in(activitySearchRequest.getAreaIds());
		if (predicate != null) {
